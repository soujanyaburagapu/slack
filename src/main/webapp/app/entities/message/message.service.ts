...
export class MessageService {
    ...
    protected convertDateFromClient(message: IMessage): IMessage {
        const copy: IMessage = Object.assign({}, message, {
            timeStamp: message.timeStamp != null && message.timeStamp.isValid() ? message.timeStamp.toJSON() : null,
            dateStamp: message.dateStamp != null && message.dateStamp.isValid() ? message.dateStamp.format(DATE_FORMAT) : null
        });
        return copy;
    }
    ...
    protected convertDateFromServer(res: EntityResponseType): EntityResponseType {
        if (res.body) {
            ...
            res.body.dateStamp = res.body.dateStamp != null ? moment(res.body.dateStamp) : null;
        }
        return res;
    }
    ...
    protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
        if (res.body) {
            res.body.forEach((message: IMessage) => {
                message.timeStamp = message.timeStamp != null ? moment(message.timeStamp) : null;
                message.dateStamp = message.dateStamp != null ? moment(message.dateStamp) : null;
            });
        }
        return res;
    }
